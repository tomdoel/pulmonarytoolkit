classdef MimStorageFormat
    % MimStorageFormat. Describes how a cached file will be stored
    %
    %   When storing data in the cache, this enumeration can be used to 
    %   choose the file format
    %
    %    Mat           - Store as a Matlab .mat file 
    %    Xml           - Convert to an XML file using a structured format
    %                    that can be loaded back into the original data
    %                    using CoreLoadXml(). Use this format when you wat
    %    SimplifiedXml - Save using a more compact XML format that uses
