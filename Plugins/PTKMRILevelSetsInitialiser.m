classdef TDMRILevelSetsInitialiser < TDPlugin
    % TDMRILevelSetsInitialiser. Plugin for producing an initialisation for
    ...
    methods (Static)
        function results = RunPlugin(dataset, reporting)
            ...
            left_and_right_lungs = dataset.GetResult('TDLeftAndRightLungsInitialiser');
            roi = dataset.GetResult('TDLungROI');
            left_roi = TDGetLeftLungROIFromLeftAndRightLungs(roi, left_and_right_lungs, reporting);
            right_roi = TDGetRightLungROIFromLeftAndRightLungs(roi, left_and_right_lungs, reporting);
            results = dataset.GetTemplateImage(TDContext.LungROI);
            results_left = TDMRILevelSetsInitialiser.ProcessLevelSets(left_roi, left_and_right_lungs, 2, is_gas, reporting);
            results_right = TDMRILevelSetsInitialiser.ProcessLevelSets(right_roi, left_and_right_lungs, 1, is_gas, reporting);
        ...
        function results = ProcessLevelSets(lung_roi, left_and_right_lungs, mask_colour, is_gas, reporting)
            ...
            results = TDFillCoronalHoles(lung_mask, is_right, reporting);
